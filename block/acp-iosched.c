/*
 * Alleviate Conflict i/o scheduler Plus.
 *
...
struct sub_queue {
	struct list_head fifo_list[2]; // for read
	struct rb_tree sort_list[2]; // for write
};
...
static void ac_add_request(struct request_queue *q, struct request *rq)
{
	...
	const rb_root root = ad->sq[idx].sort_list[sync];
