#include <linux/blkdev.h>
#include <linux/elevator.h>
#include <linux/bio.h>
#include <linux/module.h>
#include <linux/init.h>
#include <linux/slab.h>
#include <linux/version.h>

enum { ASYNC, SYNC };

/* Tunables */
static const int sync_read_expire = (HZ / 16) * 5;	/* max time before a sync read is submitted. */
static const int sync_write_expire = (HZ / 8) * 23;	/* max time before a sync write is submitted. */

static const int async_read_expire = HZ * 4;	/* ditto for async, these limits are SOFT! */
static const int async_write_expire = HZ * 16;	/* ditto for async, these limits are SOFT! */

static const int writes_starved = 2;		/* max times reads can starve a write */
static const int async_reads_starved = 4;	/* max times sync reads can starve async reads */
static const int fifo_batch     = 1;		/* # of sequential requests treated as one by the above parameters. For throughput. */

static const int fifo_size = 4096; // default bundling size


/* Elevator data */
struct lb_data {
	/* Request queues */
	struct list_head fifo_list[2][2];
	/* Attributes */
	unsigned int batched;
	unsigned int starved;
	unsigned int reads_starved;

	/* Settings */
	int fifo_expire[2][2];
	int fifo_batch;
	int writes_starved;
	int async_reads_starved;
	int fifo_size[2][2]; // for logical bundling
	
};

static void
lb_merged_requests(struct request_queue *q, struct request *rq,
		    struct request *next)
{
	/*
	 * If next expires before rq, assign its expire time to rq
	 * and move into next position (next will be deleted) in fifo.
	 */
	if (!list_empty(&rq->queuelist) && !list_empty(&next->queuelist)) {
		if (time_before(next->fifo_time, rq->fifo_time)) {
			list_move(&rq->queuelist, &next->queuelist);
			rq->fifo_time = next->fifo_time;
		}
	}

	/* Delete next request */
	rq_fifo_clear(next);
}


// add req to queue along with request type.
static void
lb_add_request(struct request_queue *q, struct request *rq)
{
	struct lb_data *ld = q->elevator->elevator_data; 
	const int sync = rq_is_sync(rq);
	const int data_dir = rq_data_dir(rq);

	/*
	 * Add request to the proper fifo list and set its
	 * expire time.
	 */
	rq->fifo_time = jiffies + ld->fifo_expire[sync][data_dir];
	ld->fifo_size[sync][data_dir] = blk_rq_size(rq);
	list_add_tail(&rq->queuelist, &ld->fifo_list[sync][data_dir]);
}

static struct request *
lb_expired_request(struct lb_data *ld, int sync, int data_dir)
{
	struct list_head *list = &ld->fifo_list[sync][data_dir];
	struct request *rq;

	if (list_empty(list))
		return NULL;

	/* Retrieve request */
	rq = rq_entry_fifo(list->next);

	/* Request has expired */
	if (time_after(jiffies, rq->fifo_time))
		return rq;

	return NULL;
}

static struct request *
lb_choose_expired_request(struct lb_data *ld)
{
	struct request *rq;

	/*
	 * Check expired requests.
	 * Asynchronous requests have priority over synchronous.
	 * Write requests have priority over read.
	 */

	rq = lb_expired_request(ld, ASYNC, WRITE);
	if (rq)
		return rq;
	rq = lb_expired_request(ld, ASYNC, READ);
	if (rq) {
		ld->reads_starved = 0;
		return rq;
	}

	rq = lb_expired_request(ld, SYNC, WRITE);
	if (rq)
		return rq;
	rq = lb_expired_request(ld, SYNC, READ);
	if (rq)
		return rq;


	return NULL;
}


static struct request *
lb_choose_request(struct lb_data *ld, int data_dir)
{
	struct list_head *sync = ld->fifo_list[SYNC];
	struct list_head *async = ld->fifo_list[ASYNC];

	/*
	 * Retrieve request from available fifo list.
	 * Synchronous requests have priority over asynchronous.
	 * Read requests have priority over write.
	 */	
	if (!list_empty(&sync[data_dir])) { // sync read
		if (ld->reads_starved <= ld->async_reads_starved || data_dir == WRITE) {
			if (data_dir == READ && !list_empty(&async[data_dir])) {
				ld->reads_starved++;
			}
			return rq_entry_fifo(sync[data_dir].next);
		}
	}
	if (!list_empty(&async[data_dir])) { //async read
		if (data_dir == READ) {
			ld->reads_starved = 0;
		}
		return rq_entry_fifo(async[data_dir].next);
	}

	if (!list_empty(&sync[!data_dir])) // sync write
		return rq_entry_fifo(sync[!data_dir].next);
	if (!list_empty(&async[!data_dir])) // async write
		return rq_entry_fifo(async[!data_dir].next);

	return NULL;
}


static inline void
lb_dispatch_request(struct lb_data *ld, struct request *rq)
{
	const int sync = rq_is_sync(rq);
	const int data_dir = rq_data_dir(rq);
	/*
	 * Remove the request from the fifo list
	 * and dispatch it.
	 */
	rq_fifo_clear(rq);
	elv_dispatch_add_tail(rq->q, rq);

	ld->batched++;
	ld->fifo_size[sync][data_dir] += blk_rq_bytes(rq); //added

	if (rq_data_dir(rq)) {
		ld->starved = 0;
	} else {
		if (!list_empty(&ld->fifo_list[SYNC][WRITE]) || 
				!list_empty(&ld->fifo_list[ASYNC][WRITE]))
			ld->starved++;
	}
}

static int
lb_dispatch_requests(struct request_queue *q, int force)
{
	struct lb_data *ld = q->elevator->elevator_data;
	struct request *rq = NULL;
	int data_dir = READ;

	/*
	 * Retrieve any expired request after a batch of
	 * sequential requests.
	 */
	if (ld->batched > ld->fifo_batch) {
		ld->batched = 0;
		rq = lb_choose_expired_request(ld);
	}

	/* Retrieve request */
	if (!rq) {
		if (ld->starved > ld->writes_starved)
			data_dir = WRITE;

		rq = lb_choose_request(ld, data_dir);
		if (!rq)
			return 0;
	}

	/* Dispatch request */
	lb_dispatch_request(ld, rq);

	return 1;
}

static struct request *
lb_former_request(struct request_queue *q, struct request *rq)
{
	struct lb_data *ld = q->elevator->elevator_data;
	const int sync = rq_is_sync(rq);
	const int data_dir = rq_data_dir(rq);

	if (rq->queuelist.prev == &ld->fifo_list[sync][data_dir])
		return NULL;

	/* Return former request */
	return list_entry(rq->queuelist.prev, struct request, queuelist);
}

static struct request *
lb_latter_request(struct request_queue *q, struct request *rq)
{
	struct lb_data *ld = q->elevator->elevator_data;
	const int sync = rq_is_sync(rq);
	const int data_dir = rq_data_dir(rq);

	if (rq->queuelist.next == &ld->fifo_list[sync][data_dir])
		return NULL;

	/* Return latter request */
	return list_entry(rq->queuelist.next, struct request, queuelist);
}

static int lb_init_queue(struct request_queue *q, struct elevator_type *e)
{
	struct lb_data *ld;
	struct elevator_queue *eq;

	eq = elevator_alloc(q, e);
	if (!eq)
		return -ENOMEM;
	
	/* Allocate structure */
	ld = kmalloc_node(sizeof(*ld), GFP_KERNEL, q->node);
	if (!ld) {
		kobject_put(&eq->kobj);
		return -ENOMEM;
	}

	eq->elevator_data = ld;
	/* Initialize fifo lists */
	INIT_LIST_HEAD(&ld->fifo_list[SYNC][READ]);
	INIT_LIST_HEAD(&ld->fifo_list[SYNC][WRITE]);
	INIT_LIST_HEAD(&ld->fifo_list[ASYNC][READ]);
	INIT_LIST_HEAD(&ld->fifo_list[ASYNC][WRITE]);

	spin_lock_irq(q->queue_lock);	
	/* Initialize data */
	q->elevator = eq;
	ld->batched = 0;
	ld->starved = 0;
	ld->reads_starved = 0;
	ld->fifo_expire[SYNC][READ] = sync_read_expire;
	ld->fifo_expire[SYNC][WRITE] = sync_write_expire;
	ld->fifo_expire[ASYNC][READ] = async_read_expire;
	ld->fifo_expire[ASYNC][WRITE] = async_write_expire;
	ld->fifo_batch = fifo_batch;
	ld->writes_starved = writes_starved;
	ld->async_reads_starved = async_reads_starved;
	spin_unlock_irq(q->queue_lock);
	
	return 0;
}

static void
lb_exit_queue(struct elevator_queue *e)
{
	struct lb_data *ld = e->elevator_data;

	BUG_ON(!list_empty(&ld->fifo_list[SYNC][READ]));
	BUG_ON(!list_empty(&ld->fifo_list[SYNC][WRITE]));
	BUG_ON(!list_empty(&ld->fifo_list[ASYNC][READ]));
	BUG_ON(!list_empty(&ld->fifo_list[ASYNC][WRITE]));

	/* Free structure */
	kfree(ld);
}

/*
 * sysfs code
 */

static ssize_t
lb_var_show(int var, char *page)
{
	return sprintf(page, "%d\n", var);
}

static ssize_t
lb_var_store(int *var, const char *page, size_t count)
{
	char *p = (char *) page;

	*var = simple_strtol(p, &p, 10);
	return count;
}

#define SHOW_FUNCTION(__FUNC, __VAR, __CONV)				\
static ssize_t __FUNC(struct elevator_queue *e, char *page)		\
{									\
	struct lb_data *ld = e->elevator_data;			\
	int __data = __VAR;						\
	if (__CONV)							\
		__data = jiffies_to_msecs(__data);			\
	return lb_var_show(__data, (page));			\
}
SHOW_FUNCTION(lb_sync_read_expire_show, ld->fifo_expire[SYNC][READ], 1);
SHOW_FUNCTION(lb_sync_write_expire_show, ld->fifo_expire[SYNC][WRITE], 1);
SHOW_FUNCTION(lb_async_read_expire_show, ld->fifo_expire[ASYNC][READ], 1);
SHOW_FUNCTION(lb_async_write_expire_show, ld->fifo_expire[ASYNC][WRITE], 1);
SHOW_FUNCTION(lb_fifo_batch_show, ld->fifo_batch, 0);
SHOW_FUNCTION(lb_writes_starved_show, ld->writes_starved, 0);
SHOW_FUNCTION(lb_async_reads_starved_show, ld->async_reads_starved, 0);
#undef SHOW_FUNCTION

#define STORE_FUNCTION(__FUNC, __PTR, MIN, MAX, __CONV)			\
static ssize_t __FUNC(struct elevator_queue *e, const char *page, size_t count)	\
{									\
	struct lb_data *ld = e->elevator_data;			\
	int __data;							\
	int ret = lb_var_store(&__data, (page), count);		\
	if (__data < (MIN))						\
		__data = (MIN);						\
	else if (__data > (MAX))					\
		__data = (MAX);						\
	if (__CONV)							\
		*(__PTR) = msecs_to_jiffies(__data);			\
	else								\
		*(__PTR) = __data;					\
	return ret;							\
}
STORE_FUNCTION(lb_sync_read_expire_store, &ld->fifo_expire[SYNC][READ], 0, INT_MAX, 1);
STORE_FUNCTION(lb_sync_write_expire_store, &ld->fifo_expire[SYNC][WRITE], 0, INT_MAX, 1);
STORE_FUNCTION(lb_async_read_expire_store, &ld->fifo_expire[ASYNC][READ], 0, INT_MAX, 1);
STORE_FUNCTION(lb_async_write_expire_store, &ld->fifo_expire[ASYNC][WRITE], 0, INT_MAX, 1);
STORE_FUNCTION(lb_fifo_batch_store, &ld->fifo_batch, 0, INT_MAX, 0);
STORE_FUNCTION(lb_writes_starved_store, &ld->writes_starved, 0, INT_MAX, 0);
STORE_FUNCTION(lb_async_reads_starved_store, &ld->async_reads_starved, 0, INT_MAX, 0);
#undef STORE_FUNCTION

#define DD_ATTR(name) \
	__ATTR(name, S_IRUGO|S_IWUSR, lb_##name##_show, \
				      lb_##name##_store)

static struct elv_fs_entry lb_attrs[] = {
	DD_ATTR(sync_read_expire),
	DD_ATTR(sync_write_expire),
	DD_ATTR(async_read_expire),
	DD_ATTR(async_write_expire),
	DD_ATTR(fifo_batch),
	DD_ATTR(writes_starved),
	DD_ATTR(async_reads_starved),
	__ATTR_NULL
};

static struct elevator_type iosched_lb = {
	.ops = {
		.elevator_merge_req_fn		= lb_merged_requests,
		.elevator_dispatch_fn		= lb_dispatch_requests,
		.elevator_add_req_fn		= lb_add_request,
		.elevator_former_req_fn		= lb_former_request,
		.elevator_latter_req_fn		= lb_latter_request,
		.elevator_init_fn		= lb_init_queue,
		.elevator_exit_fn		= lb_exit_queue,
	},

	.elevator_attrs = lb_attrs,
	.elevator_name = "lb",
	.elevator_owner = THIS_MODULE,
};

static int __init lb_init(void)
{
	/* Register elevator */
	elv_register(&iosched_lb); 

	return 0;
}

static void __exit lb_exit(void)
{
	/* Unregister elevator */
	elv_unregister(&iosched_lb);
}

module_init(lb_init);
module_exit(lb_exit);

MODULE_AUTHOR("Kairi OKUMURA");
MODULE_LICENSE("GPL");
MODULE_DESCRIPTION("Logical Bundling I/O Scheduler");

